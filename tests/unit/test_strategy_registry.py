...
class MockStrategy(RoutingStrategy):
    ...
    def __init__(self, name: str = "mock", deployment_to_return: Optional[Dict] = None):
        self._name = name
        self._deployment_to_return = deployment_to_return or {"model_name": "test-model"}
        self.call_count = 0
...
class TestRoutingPipeline:
    ...
    def test_pipeline_fallback_on_error(self):
        ...
        pipeline = RoutingPipeline(registry, default_strategy=fallback, emit_telemetry=False)
    ...
    def test_pipeline_no_strategy(self):
        ...
        pipeline = RoutingPipeline(registry, default_strategy=default, emit_telemetry=False)
...
class TestConcurrencySafety:
    ...
    def test_concurrent_registration(self):
        ...
        assert len(errors) == 0
        assert len(registry.list_strategies()) == num_threads * num_strategies_per_thread
    ...
    def test_concurrent_selection(self):
        ...
        def select_strategies(thread_id: int):
            local_results = []
            try:
                for i in range(num_selections_per_thread):
                    key = f"user:{thread_id}-{i}"
...
class TestSingletonAccess:
    ...
    def test_pipeline_creation_with_custom_default(self):
        ...
        pipeline = RoutingPipeline(registry, default_strategy=mock_default, emit_telemetry=False)
