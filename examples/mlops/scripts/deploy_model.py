...
@click.option(
    "--model-stage", default="Production", help="Model stage (Production, Staging)"
)
@click.option(
    "--s3-bucket", envvar="TARGET_S3_BUCKET", required=True, help="Target S3 bucket"
)
...
def deploy_model(
    model_name: str, model_stage: str, s3_bucket: str, s3_prefix: str, local_path: str
):
    ...
    try:
        mlflow.artifacts.download_artifacts(
            artifact_uri=model_uri, dst_path=str(local_model_path)
        )
    ...
    try:
        ...
        for file_path in local_model_path.rglob("*"):
            if file_path.is_file():
                s3_key = (
                    f"{s3_prefix}{model_name}/{file_path.relative_to(local_model_path)}"
                )
